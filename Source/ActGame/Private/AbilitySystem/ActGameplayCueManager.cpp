...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ActGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EActEditorLoadMode
{
...
namespace ActGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Act.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via ActGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UActGameplayCueManager::DumpGameplayCues));

	static EActEditorLoadMode LoadMode = EActEditorLoadMode::LoadUpfront;
}
...
UActGameplayCueManager::UActGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UActGameplayCueManager* UActGameplayCueManager::Get()
{
	return Cast<UActGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UActGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UActGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogAct, Warning, TEXT("UActGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UActGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (ActGameplayCueManagerCvars::LoadMode)
	{
	case EActEditorLoadMode::LoadUpfront:
		return true;
	case EActEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EActEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UActGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UActGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UActGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UActGameplayCueManager* GCM = Cast<UActGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogAct, Error, TEXT("DumpGameplayCues failed. No UActGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogAct, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogAct, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogAct, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogAct, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogAct, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogAct, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogAct, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogAct, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogAct, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogAct, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogAct, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogAct, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UActGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UActGameplayCueManager* StrongThis = Get())
					{
...
void UActGameplayCueManager::HandlePostGarbageCollect()
{
...
void UActGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogAct, Warning, TEXT("UActGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UActGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (ActGameplayCueManagerCvars::LoadMode)
	{
	case EActEditorLoadMode::LoadUpfront:
...
void UActGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UActGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UActGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UActGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UActGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UActGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
