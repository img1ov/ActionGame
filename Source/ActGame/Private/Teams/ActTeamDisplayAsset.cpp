...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ActTeamDisplayAsset)

void UActTeamDisplayAsset::ApplyToMaterial(UMaterialInstanceDynamic* Material)
{
	if (Material)
	{
...
void UActTeamDisplayAsset::ApplyToMeshComponent(UMeshComponent* MeshComponent)
{
	if (MeshComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UActTeamDisplayAsset::ApplyToNiagaraComponent(UNiagaraComponent* NiagaraComponent)
{
	if (NiagaraComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UActTeamDisplayAsset::ApplyToActor(AActor* TargetActor, bool bIncludeChildActors)
{
...
#if WITH_EDITOR
void UActTeamDisplayAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (UActTeamSubsystem* TeamSubsystem : TObjectRange<UActTeamSubsystem>())
	{
