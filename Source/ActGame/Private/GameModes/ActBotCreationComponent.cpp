...
#include "GameModes/ActBotCreationComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ActBotCreationComponent)

UActBotCreationComponent::UActBotCreationComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UActBotCreationComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	ExperienceComponent->CallOrRegister_OnExperienceLoaded_LowPriority(FOnActExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::OnExperienceLoaded));
}

void UActBotCreationComponent::OnExperienceLoaded(const UActExperienceDefinition* Experience)
{
#if WITH_SERVER_CODE
	if (HasAuthority())
	{
...
void UActBotCreationComponent::ServerCreateBots_Implementation()
{
	...
	if (GIsEditor)
	{
		const UActDeveloperSettings* DeveloperSettings = GetDefault<UActDeveloperSettings>();
...
FString UActBotCreationComponent::CreateBotName(int32 PlayerIndex)
{
	FString Result;
...
void UActBotCreationComponent::SpawnOneBot()
{
	...
	{
		AActGameMode* GameMode = GetGameMode<AActGameMode>();
		check(GameMode);
		...
		{
			if (UActPawnExtensionComponent* PawnExtComponent = NewController->GetPawn()->FindComponentByClass<UActPawnExtensionComponent>())
			{
...
void UActBotCreationComponent::RemoveOneBot()
{
	if (SpawnedBotList.Num() > 0)
	{
		...
		if (BotToRemove)
		{
			...
			{
				if (UActHealthComponent* HealthComponent = UActHealthComponent::FindHealthComponent(ControlledPawn))
				{
