...
namespace UE::ComboGraph
{
	...
	/**
	 * Simple helper to return either a FVector2f (for 5.6 and higher) or FVector2D (for 5.5 and below), to work around
	 * the engine deprecation of FVector2D for a few versions.
	 */
	static FVector2f ToVector2ForD(const float InX, const float InY)
	{
		return FVector2f(InX, InY);
	}
#else
	/**
	 * Simple helper to return either a FVector2f (for 5.6 and higher) or FVector2D (for 5.5 and below), to work around
	 * the engine deprecation of FVector2D for a few versions.
	 */
	static FVector2D ToVector2ForD(const float InX, const float InY)
	{
		...
		if (Pin)
		{
			...
			for (int32 Idx = 0; Idx < TempLinkedTo.Num(); Idx++)
			{
				...
				if (GraphNode && BBoxTree.Children.Num() > 0)
				{
					...
					const float NewPosX = BBoxTree.Children[BBoxIndex].SubGraphBBox.X / 2 - GraphNode->DEPRECATED_NodeWidget.Pin()->GetDesiredSize().X / 2 + PosX;
					GraphNode->DEPRECATED_NodeWidget.Pin()->MoveTo(UE::ComboGraph::ToVector2ForD(NewPosX, PosY), NodeFilter);
		...
		if (Pin)
		{
			...
			for (int32 Idx = 0; Idx < TempLinkedTo.Num(); Idx++)
			{
				...
				if (GraphNode && BBoxTree.Children.Num() > 0)
				{
					...
					const float NewPosY = BBoxTree.Children[BBoxIndex].SubGraphBBox.Y / 2 - GraphNode->DEPRECATED_NodeWidget.Pin()->GetDesiredSize().Y / 2 + PosY;
					GraphNode->DEPRECATED_NodeWidget.Pin()->MoveTo(UE::ComboGraph::ToVector2ForD(PosX, NewPosY), NodeFilter);
...
void UComboGraphEdGraph::AutoArrange(const bool bVertical)
{
	...
	if (bVertical)
	{
		...
		RootNode->DEPRECATED_NodeWidget.Pin()->MoveTo(UE::ComboGraph::ToVector2ForD(NewRootPosX, 0), NodeFilter);
	}
	else
	{
		...
		RootNode->DEPRECATED_NodeWidget.Pin()->MoveTo(UE::ComboGraph::ToVector2ForD(0, NewRootPosY), NodeFilter);
	}
