...
FBulletActionInfo UBulletActionCenter::AcquireActionInfo()
{
    if (ActionInfoPool.Num() > 0)
    {
        ...
        // Always return a clean payload. Callers typically overwrite fields, but defaults are safer.
...
{
    // FBulletActionInfo is a value type; we keep a copy to reuse allocations.
