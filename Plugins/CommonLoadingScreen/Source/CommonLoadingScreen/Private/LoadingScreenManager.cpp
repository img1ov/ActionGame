...
void ULoadingScreenManager::ShowLoadingScreen()
{
	...
	else
	{
		...
		if (LoadingScreenWidgetClass)
		{
			...
			{
				UE_LOG(LogLoadingScreen, Error, TEXT("Failed to create the loading screen widget %s, falling back to placeholder."), *Settings->LoadingScreenWidget.ToString());
				LoadingScreenWidget = SNew(SThrobber);
			}
		}
		else
		{
			// Avoid calling CreateWidget with a null class when the project hasn't configured a widget yet.
			UE_LOG(LogLoadingScreen, Verbose, TEXT("LoadingScreenWidget is not set. Falling back to placeholder."));
			LoadingScreenWidget = SNew(SThrobber);
		}
